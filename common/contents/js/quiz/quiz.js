...
function switchStringNumListToArray(t, e) {
  ...
  $ts.loadScriptFile("../common/libs/js/dragdropCore.js"), $ts.loadScriptFile("../common/contents/js/quiz/dragDrop.js"), $ts.loadScriptFile("../common/contents/js/quiz/dragLine.js"), window.$quiz = function (t) {
    return new i(t)
  }
}(),
function () {
  function e(s) {
    ...
    function i(t, e) {
      if (t.classList.contains("complete")) s.option.toggle && (! function t(e) {
        ...
        if (t.hasAttribute("data-group-index"))
          for (var n = a.obj, i = 0; i < n.length; i++) t.getAttribute("data-group-index") === n[i].getAttribute("data-group-index") && (a.completeCount++, n[i].classList.add("complete"));
        else
          for (var i in a.completeCount++, t.classList.add("complete"), e) e[i].classList.add("on");
...
function () {
  function e(s) {
    ...
    function i(t, e) {
      if (a.reset(), t.hasAttribute("data-answer"))
        for (var n in a.completeCount++, e) e[n].classList.add("on");
      else
        for (var n in e) e[n].classList.add("on");
    ...
    function o(t, e) {
      if (t.classList.contains("checked")) c(t);
      else if (t.classList.contains("complete")) c(t);
      else {
    ...
    this.container = s.container, this.obj = $ts.getEl("[data-" + s.type + "-obj]", this.container), this.target = $ts.getEl("[data-" + s.type + "-target]", this.container), this.totalCompleteCount = $ts.getEl("[data-answer]", this.container).length, this.completeCount = 0, this.showAnswer = function () {
        ...
        for (var e in this.obj)
          if (this.obj[e].hasAttribute("data-answer")) {
            var n = s.getIndex({
                obj: a.obj[e],
                type: "check"
              }),
...
function () {
  function e(e) {
    var n = this,
      i = $ts.getEl("[data-drag-drop-container]", e.container, 0),
      t = $ts.getEl("[data-drag-element]", i),
      o = $ts.getEl("[data-drop-area-element]", i),
      s = new DragDrop({
        containerElement: i,
        dragElements: t,
        dropAreaElements: o
...
function () {
  function e(o) {
    this.answers = [], this.dots = [], this.btns = [], this.svg = void 0, this.lines = [], Object.defineProperties(this, {
      ...
      for (var e = 0; e < this.len_dots; e++)
        if (t === this.dots[e].dotIndex) return this.dots[e]
    }, this.getDotPosition = function (t) {
      var e = $ts.getSize(t),
        n = $ts.getSize(o.container);
      return {
        x: e.left - n.left + e.width / 2,
        y: e.top - n.top + e.width / 2
      }
